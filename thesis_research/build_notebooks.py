...
def build_phase2_notebook():
    ...
    cells.append(code([
        ...
        '',
        'results = {n: defaultdict(list) for n in [',
        '    "CertGraph", "Homogeneous GCN", "Hetero-GCN", "Hetero-SAGE",',
        '    "MLP", "Rule-Based", "Random Forest"',
        ']}',
        'all_histories = []',
        'best_model, best_f1 = None, 0',
        'last_cg_results = None',
        '',
        'def train_gnn_baseline(model_class, train_loader, test_loader, is_certgraph=False):',
        '    s0 = dataset[0][0]',
        '    if is_certgraph:',
        '        m = model_class(metadata=s0.metadata(), hidden_channels=HIDDEN_DIM,',
        '                        out_channels=OUT_DIM, num_classes=NUM_CLASSES,',
        '                        num_heads=NUM_HEADS, dropout=DROPOUT)',
        '    else:',
        '        m = model_class(metadata=s0.metadata(), hidden_channels=HIDDEN_DIM * NUM_HEADS,',
        '                        out_channels=OUT_DIM, num_classes=NUM_CLASSES, dropout=DROPOUT)',
        '    with torch.no_grad():',
        '        m.eval(); _ = m(s0.x_dict, s0.edge_index_dict)',
        '    m.train()',
        '    opt = torch.optim.Adam(m.parameters(), lr=LR, weight_decay=1e-4)',
        '    history = {"loss": [], "train_f1": []}',
        '    for epoch in range(1, EPOCHS+1):',
        '        m.train(); total_loss = 0; preds_e, labs_e = [], []',
        '        for batch in train_loader:',
        '            opt.zero_grad()',
        '            logits = m(batch.x_dict, batch.edge_index_dict)',
        ...
        '    # ── CertGraph ──',
        '    print("  [CertGraph]")',
        '    cg_model, cg_f1, cg_acc, cg_preds, cg_labels, cg_history = train_gnn_baseline(CertGraph, train_loader, test_loader, is_certgraph=True)',
        '    results["CertGraph"]["f1"].append(cg_f1)',
        '    results["CertGraph"]["acc"].append(cg_acc)',
        '    all_histories.append(cg_history)',
        '    if cg_f1 > best_f1: best_f1, best_model = cg_f1, cg_model',
        '    last_cg_results = {"preds": cg_preds, "labels": cg_labels}',
        '    print(f"    CertGraph \u2192 F1: {cg_f1:.4f} | Acc: {cg_acc:.4f}")',
        '',
        '    # ── Homogeneous GCN baseline ──',
        '    print("  [Homogeneous GCN]")',
        '    _, gcn_f1, gcn_acc, _, _, _ = train_gnn_baseline(GCNBaseline, train_loader, test_loader, is_certgraph=False)',
        '    results["Homogeneous GCN"]["f1"].append(gcn_f1)',
        '    results["Homogeneous GCN"]["acc"].append(gcn_acc)',
        '    print(f"    GCN       \u2192 F1: {gcn_f1:.4f}")',
        '',
        '    # ── Hetero-GCN baseline ──',
        '    print("  [Hetero-GCN]")',
        '    _, hgcn_f1, hgcn_acc, _, _, _ = train_gnn_baseline(HeteroGCNBaseline, train_loader, test_loader, is_certgraph=False)',
        '    results["Hetero-GCN"]["f1"].append(hgcn_f1)',
        '    results["Hetero-GCN"]["acc"].append(hgcn_acc)',
        '    print(f"    HeteroGCN \u2192 F1: {hgcn_f1:.4f}")',
        '',
        '    # ── Hetero-SAGE baseline ──',
        '    print("  [Hetero-SAGE]")',
        '    _, hsage_f1, hsage_acc, _, _, _ = train_gnn_baseline(HeteroSAGEBaseline, train_loader, test_loader, is_certgraph=False)',
        '    results["Hetero-SAGE"]["f1"].append(hsage_f1)',
        '    results["Hetero-SAGE"]["acc"].append(hsage_acc)',
        '    print(f"    HeteroSAGE\u2192 F1: {hsage_f1:.4f}")',
        '',
        '    # ── MLP baseline ──',
    ...
    cells.append(code([
        ...
        'colors = ["#FF6B6B", "#4ECDC4", "#FFA07A", "#45B7D1", "#E8A0BF", "#FFD93D", "#95E1D3"]',
        '',
    ...
    # ADSynth Domain Generalization / Transfer Learning
    cells.append(md([
    ...
    # Multi-Tool Comparison Baseline
    cells.append(md([
    ...
    # GNN Sensitivity & Robustness Analysis
    cells.append(md([
    ...
    # Real-World AD Topology Case Study (GOAD)
    cells.append(md([
    ...
    # External Community-Provided Dataset (m4lwhere)
    cells.append(md([
